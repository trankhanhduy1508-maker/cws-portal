...
if not defined CWS_SUPABASE_KEY (
  echo [ERROR] Missing CWS_SUPABASE_KEY. Secrets must come from the host environment.
  exit /b 1
)
if not defined CWS_B2_KEY_ID (
  echo [ERROR] Missing CWS_B2_KEY_ID. Secrets must come from the host environment.
  exit /b 1
)
if not defined CWS_B2_APP_KEY (
  echo [ERROR] Missing CWS_B2_APP_KEY. Secrets must come from the host environment.
  exit /b 1
)
