...
class LoadOrdersRepository implements IRenderOrdersRepository {
  ...
  async updateResult(): Promise<RenderOrder | null> { return null; }
  async updateLockedResult(): Promise<RenderOrder | null> { return null; }
  async unlockResult(): Promise<RenderOrder | null> { return null; }
  ...
  async attachPayment(): Promise<RenderOrder | null> { return null; }
  async markPaymentPaid(): Promise<void> { return undefined; }
...
class LoadWorkerGateway {
  ...
  async createInternalJobWithProbeTask(params: { internalJobId: string }): Promise<string> {
  ...
  async countOnlineWorkers(): Promise<number> { return this.onlineWorkers; }
  ...
  async getTotalFrames(): Promise<number | null> { return null; }
...
describe('CWS real Nest load simulation (safe, no external writes)', () => {
  ...
  beforeAll(async () => {
    ...
    const module = await Test.createTestingModule({ imports: [AppModule] })
        ...
        getClient: () => ({
          auth: {
            getUser: async (token: string) => ({
              data: { user: token?.startsWith('load-') ? { id: token } : null },
              error: null,
            }),
          },
        }),
      ...
      .useValue({ notify: async () => ({}), logWorkerEvent: async () => ({}) })
      ...
      .useValue({
        assertOwned: async (fileRef: string, customerId: string) => {
          ...
          if (duplicate && customerId === `load-duplicate-${duplicate[1]}`) return;
          const input = /input-(\d+)-(\d+)\.blend$/.exec(fileRef);
          if (input && customerId === `load-user-${input[1]}-${input[2]}`) return;
    ...
    for (const customers of [10, 25, 50, 100]) {
      ...
      const submitted = await Promise.all(
        Array.from({ length: customers }, async (_, index) => {
          ...
          try {
            ...
            return { status: 0, body: {}, customerToken, error: String(error) };
          }
        }),
      );

      const transportErrors = submitted.filter((response) => 'error' in response);
      ...
      const claimResults = await Promise.all(
        Array.from(
          ...
          (_, index) => workers.claim(`load-worker-${index % workers.onlineWorkers}`),
        ),
      );
