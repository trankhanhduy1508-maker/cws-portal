...
export class WorkerAuthService {
  ...
  async authenticate(request: Request): Promise<AuthenticatedWorker> {
    ...
    if (scheme !== 'Worker' || !token || !workerId || !timestamp || !nonce || !signature) {
    ...
    if (proofError) throw new UnauthorizedException('Invalid Worker authentication');
    ...
    const { data, error } = await client
      ...
      .maybeSingle();
    if (error || !data) throw new UnauthorizedException('Invalid Worker authentication');
    ...
    if (identity.credential_hash !== sha256Hex(token)) {
      throw new UnauthorizedException('Invalid Worker authentication');
    }

    const { error: nonceError } = await client.from('worker_auth_nonces').insert({
      worker_id: workerId,
      nonce,
      expires_at: new Date(Date.now() + WORKER_NONCE_TTL_SECONDS * 1000).toISOString(),
    });
    if (nonceError) {
