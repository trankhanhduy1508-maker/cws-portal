...
- Customer and Admin download calls now send the Supabase Bearer token only in the Authorization header, receive a short-lived signed URL via an authenticated JSON exchange, then navigate directly to B2. The bearer token never appears in the URL.
...
The signed URL exchange avoids requiring CORS-enabled B2 fetches while preserving Backend ownership checks and download audit issuance.
GitHub Actions #272 PASS on code head 4cf23a3: backend build/tests and frontend build/lint.
