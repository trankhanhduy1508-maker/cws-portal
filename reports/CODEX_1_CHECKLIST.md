...
- [x] Kiểm tra Build — xác minh thật (không chỉ tin log cũ): `cd backend && npm run build` (nest build) PASS; `npm run lint` (oxlint) + `npm run build` (vite build) ở root PASS; `cd backend && npm test` (jest) 9/9 PASS. Chạy lại ngay trong phiên này để xác nhận, không dùng kết quả cũ.
