...
- **Rủi ro OOM upload (2026-07-31, tự phát hiện):** `POST /files/upload` không giới hạn `fileSize` ở tầng multer — sửa bằng `limits.fileSize` trong `FileInterceptor` + xử lý `MulterError` riêng (413) trong `HttpExceptionFilter`. Test: 29 → 31. Chi tiết: `backend/CHANGELOG.md` mục `[1.7.0]`.
- **Lỗ hổng bảo mật thứ 3 (IDOR qua WebSocket, 2026-07-31, tự phát hiện):** `/ws/jobs/:id` vẫn còn nguyên lỗ hổng IDOR y hệt REST dù đã sửa ở trên, vì là đường vào dữ liệu riêng — sửa bằng token qua query string + `resolveCustomerId()` mới. Test: 31 → 35. Chi tiết: `backend/CHANGELOG.md` mục `[1.8.0]`.
- **Regression nghiêm trọng do chính fix IDOR gây ra + presigned URL cho B2 (2026-07-31, tự phát hiện qua kiểm tra chéo):** `RenderService.js` gọi 5 route vừa khoá quyền sở hữu (`cancelJob`/`getJob`/`getJobPreview`/`approveJob`/`requestJobChanges`) mà KHÔNG đính `Authorization` header — sẽ tự chặn nhầm mọi khách đã đăng nhập ngay khi Facebook Provider bật thật. Đồng thời phát hiện `B2StorageService` trả URL công khai tĩnh (không ký/hết hạn), khiến fix IDOR vô nghĩa nếu bucket B2 thật public-read. Đã sửa cả 2: đính token vào toàn bộ call site + `getDownloadUrl()` qua query string; thêm presigned URL (`@aws-sdk/s3-request-presigner`) cho preview/download. Test: 35 → 37. Chi tiết: `backend/CHANGELOG.md` mục `[1.9.0]`.
