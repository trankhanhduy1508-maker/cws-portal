...
- [x] **[LỖ HỔNG BẢO MẬT phát hiện qua self-review sau audit — 2026-07-30]** `POST /payments/webhook` không có xác thực nào — payment_code/storage_code/amount đối chiếu không đủ vì cả 3 giá trị này chính khách hàng nhìn thấy trên QR để chuyển khoản, nên khách có thể tự gọi webhook đánh dấu PAID mà không cần chuyển tiền thật. Đã sửa: `WebhookSecretGuard` mới (header `x-webhook-secret` khớp `PAYMENT_WEBHOOK_SECRET`, fail-closed như `AdminKeyGuard`) + 3 test mới. Nhân tiện phát hiện `.github/workflows/ci.yml` chưa từng lint backend — đã thêm bước lint vào CI + tự sửa 1 lỗi eslint có sẵn (`_providerRef` không dùng). Xem `backend/CHANGELOG.md` mục [1.5.0].
- [x] **[LỖ HỔNG BẢO MẬT thứ 2 (IDOR) phát hiện qua self-review liên tiếp — 2026-07-31]** Toàn bộ route `/jobs/:id/...` (GET job/status/preview/download/logs/notifications, POST approve/cancel/request-changes, DELETE) không kiểm tra chủ sở hữu — chỉ dựa vào UUID khó đoán. Ai biết được job id là xem/huỷ/tải được job của khách khác. Đã sửa: `JobsService.assertOwnership()` mới (job có customerId bắt buộc khớp, trừ khi có `x-admin-key` hợp lệ; job chưa có customerId vẫn mở cho khách vãng lai). Cập nhật `adminApi.js`/`AdminScreen.jsx` gửi `x-admin-key` cho route preview (trước đây cố ý công khai). Thêm 6 test mới. Xem `backend/CHANGELOG.md` mục [1.6.0].
