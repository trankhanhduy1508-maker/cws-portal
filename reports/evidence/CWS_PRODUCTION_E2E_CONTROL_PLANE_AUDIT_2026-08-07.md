...
- Required production bridge state: `worker_identities` absent,
  `claim_next_resilient_task(text,integer)` absent,
  `get_claimed_task_spec(bigint,integer,text)` absent; fenced heartbeat exists.
- Supabase migration list does not contain the prepared worker migrations
  020/021/022.
...
The production backend cannot use the canonical authenticated Node Agent bridge
until migrations 020/021/022 are applied. The Windows host has no production
Worker identity, `CWS_*` runtime configuration or B2 credentials, and the
Desktop package does not contain `production_node_agent.py`. No production job,
payment, B2 mutation or fake completion was created.
