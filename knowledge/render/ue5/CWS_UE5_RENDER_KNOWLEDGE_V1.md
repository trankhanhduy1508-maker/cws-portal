...
### Native 2K gate — Blender source proven, UE5 map-transition blocker (2026-08-22)

The required quality gate is now genuine native `2560x1440`; the earlier Lanczos files remain delivery-upscale benchmarks only. Blender 5.2.0 LTS rendered `.cws_tmp/B4_JOB/BlenderReferenceNative2K/frame_0462_native2K.png` from the immutable `.cws_tmp/PhongNguRender6.blend` with Cycles. FFprobe verified `2560x1440`, `rgb24`, so the source can produce real native 2K detail. Blender's official Format documentation defines X/Y as output pixels and Percentage as scaling; native output requires 100%.

The first UE5 native2K representative script used a disposable map/material/texture, a one-frame MRQ range at frame 0030, output `2560x1440`, and the already verified `-dx11 -sm5 -DDC-ForceMemoryCache` path. UE5 passed SM5 startup and shader bootstrap, but the script's duplicate-map -> `LevelEditorSubsystem.load_level(duplicate)` transition left the duplicate world standalone and UE 5.8.1 aborted before MRQ with `World Memory Leaks: 2 leaks objects and packages`. Removing the Python duplicate return reference did not change the result. This is a script/world-transition defect, not evidence against native 2K rendering.

Decision: do not call native2K PASS yet and do not render a full sequence. Keep all 640 outputs, the 2560x1440 Lanczos benchmark and both UE logs unchanged. The next smallest test must avoid duplicate-map loading in the same editor process: use a pre-authored disposable map loaded as the startup map, or load the duplicate as startup and never call `load_level()` on it. Full evidence: `reports/evidence/CWS_UE5_NATIVE2K_GATE_2026-08-22.md`. Official references: [Blender Format](https://docs.blender.org/manual/en/latest/render/output/properties/format.html), [Epic MRQ high-resolution frames](https://dev.epicgames.com/documentation/en-us/unreal-engine/rendering-high-quality-frames-with-movie-render-queue-in-unreal-engine), and [Epic MoviePipelineOutputSetting](https://dev.epicgames.com/documentation/en-us/unreal-engine/python-api/class/MoviePipelineOutputSetting).
...
The fast downstream numbers must not be presented as `.blend -> UE5` end-to-end time. The input beauty plate was generated by `.cws_tmp/B4_JOB/render_blender_plate_sequence.py` from `PhongNguRender6.blend` using Blender/Cycles at 640x360, 4 samples, frames 432–491. Saved-frame timestamps establish at least `20m39s` for the plate stage alone; adding UE5 `17.434s` and encode `5.37s` yields an end-to-end lower bound of approximately `21m02s`, before unrecorded Blender startup overhead.
...
## MAY083 NATIVE CYCLES OPTIX FULL HD — 2026-08-22
...
## MAY083 BFUE camera/lighting/rebound gate — 2026-08-22

Native 1920x1080 MRQ diagnostic produced 60 PNG frames, but frames 0000/0030/0059 were identical black images (36,314 bytes; SHA-256 `593ABE957795AF4129D2D1E579954C4DB309330C278B989155F62C0E88BCF8EC`). BFUE source confirms camera text mapping `Pitch=rotation_y`, `Yaw=rotation_z`, `Roll=rotation_x`; UE Python positional `Rotator` arguments were also observed to reorder values in the saved actor, so use keyword arguments or a calculated look-at. UE bounds placed the skeletal actor at origin `(52.3165,23.0680,60.8221)` with extent `(76.4714,33.1631,63.2477)`. A look-at patch and added directional/point lights did not produce visual signal. A separate `BFUE_CharacterRebound_V1_Sequence` correctly rebound current camera, skeletal actor and animation, but remained black. MRQ completion and 1920x1080 resolution are proven; visual/semantic scene signal is not. Stop this camera/character-only repair family unless new evidence addresses render target alpha/world state or an explicit known-good diagnostic material. Evidence: `reports/evidence/CWS_BFUE_CAMERA_LIGHTING_REBOUND_GATE_2026-08-22.md`.

## MAY083 BFUE runtime binding / spawnable gate — 2026-08-22

The black-frame boundary was isolated with a visible `BasicShapes/Cube` probe. Earlier 1920x1080 MRQ PNGs were not merely dark: sampled frames contained one RGBA color `(0,0,0,0)` with fully transparent alpha. The cube rendered in the same map/queue, so MRQ, camera-cut evaluation, world loading and output were capable of producing visible pixels. Runtime inspection showed the skeletal component visible, not hidden in game, with mesh `Circle_013`, Single Node animation mode and five imported material slots.

A possessable camera/character sequence remained transparent after removing the probe. A materially different sequence built with UE’s supported spawnable binding path (`add_spawnable_from_instance` for camera and skeletal actor, plus a skeletal animation track) produced visible character geometry in all 60 native 1920x1080 frames. This confirms that Level actor identity/possessable binding was a real failure boundary, not an MRQ-only black-screen problem. Epic documents the same possessable/spawnable distinction and exposes `AddSpawnableFromInstance`, `AddSpawnableFromClass`, `ConvertToSpawnable` and spawnable camera creation in `LevelSequenceEditorSubsystem`.

V3 camera targeting `(0,0,60)` and reduced light intensity produced a native representative where the head and torso were visible without the probe or clipped white exposure. It still failed the practical fidelity gate: hair, eyes, glasses, clothing/texture semantics and environment were incomplete, and framing was not yet a full-body match. Evidence: `reports/evidence/CWS_BFUE_RUNTIME_BINDING_SPAWNABLE_GATE_2026-08-22.md`, `.cws_tmp/B4_JOB/BFUE_CharacterSpawnableV3Render/0030.png`, `.cws_tmp/B4_JOB/bfue_spawnable_render_report.json`.

Decision: preserve spawnable camera + spawnable skeletal actor as the next implementation base; do not repeat the old possessable-only repair family. Do not call this a production pass or goal achievement. The true `.blend -> MP4` benchmark and complete material/character/environment transfer remain unresolved.
