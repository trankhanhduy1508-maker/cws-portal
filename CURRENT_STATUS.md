...
- Admin authentication update 2026-08-06: Google OAuth + Supabase TOTP flow is **CODE/UNIT VERIFIED**; backend pre-MFA staff check is non-sensitive and all Admin data routes still require role + `aal2`. Fresh read-only evidence still finds Render `/staff/mfa-status` at HTTP 404 and no real staff AAL2 session has been exercised. After commit `83fdff7`, the latest observed Vercel READY production deployment remains commit `33dc578`; the new fleet-only Admin/customer-flow UI is not production-verified.
