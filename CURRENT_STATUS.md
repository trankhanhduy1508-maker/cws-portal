...
- Admin authentication update 2026-08-06: Google OAuth + Supabase TOTP flow is **CODE/UNIT VERIFIED**; backend pre-MFA staff check is non-sensitive and all Admin data routes still require role + `aal2`. Vercel production now has a READY deployment for commit `da6c16a` and the live bundle contains the Google/MFA flow. Full production auth remains **NOT VERIFIED** because Render still returns `404 Cannot GET /staff/mfa-status` and no real staff AAL2 session has been exercised.
