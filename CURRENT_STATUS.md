...
✅ **Auto Detect implemented (2026-08-01)**: researched Casso vs SePay from official docs (developer.sepay.vn, api.casso.vn/pricing-table) — chose **SePay** (free tier has MB Bank + webhook/API from the start; Casso free has no custom webhook, needs the 99k/month Starter plan). Built `POST /payments/webhook/sepay` (`SepayWebhookGuard` — fixed `Authorization: Apikey <key>` header per SePay's spec — → `PaymentsService.confirmViaSepayWebhook()`): filters `transferType != 'in'` safely (no error), reuses the existing `payment_notifications` table (migration 014) for idempotency/replay protection via SePay's transaction `id` (no parallel payment system created), then reuses the exact same `matchAndConfirm()` logic as the generic webhook (payment_code + storage_code + amount must all match) before setting PAID. CODE VERIFIED: 15 new tests (5 SePay service + 4 SePay guard + 6 `QrBankProvider`, covering the not-yet-configured-account / VietQR-URL-built-correctly / rounding / `confirm()`-always-rejects cases), 88/88 backend tests pass, build clean. Docs updated: `backend/BACKEND_SETUP.md` §3c (setup steps), `backend/API_DOCUMENTATION.md` (endpoint), `backend/.env.example` (`SEPAY_WEBHOOK_API_KEY`), `DECISIONS.md` (official choice).
...
⬜ RUNTIME NOT VERIFIED against a real transaction — needs Owner to (1) create a SePay account and link the real MB Bank account (agent cannot do this — requires logging into online banking), (2) configure the webhook + `SEPAY_WEBHOOK_API_KEY` on the deployed backend per `BACKEND_SETUP.md` §3c, (3) set `MB_BANK_ACCOUNT_NUMBER`/`MB_BANK_ACCOUNT_NAME` on Render.com, (4) send one real bank transfer.
