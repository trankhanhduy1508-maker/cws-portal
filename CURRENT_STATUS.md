...
⬜ RUNTIME NOT VERIFIED against a real transaction — needs Owner to (1) create a SePay account and link the real MB Bank account (agent cannot do this — requires logging into online banking), (2) configure the Webhook on SePay Dashboard (URL + Auth Type, see next message for exact values) + set `SEPAY_WEBHOOK_HMAC_SECRET` or `SEPAY_WEBHOOK_API_KEY` on Render.com, (3) set `MB_BANK_ACCOUNT_NUMBER`/`MB_BANK_ACCOUNT_NAME` on Render.com, (4) send one real bank transfer.
